...
s6a_fd_objs_init()
{
	...
	FD_DICT_SEARCH(DICT_COMMAND, CMD_BY_NAME, "Insert-Subscriber-Data-Request",
							g_fd_dict_objs.IDR);

	FD_DICT_SEARCH(DICT_COMMAND, CMD_BY_NAME, "Insert-Subscriber-Data-Answer",
							g_fd_dict_objs.IDA);
...
s6a_fd_cb_reg(void)
{
	...
	data.command = g_fd_dict_objs.IDR;
	CHECK_FCT_DO(fd_disp_register(idr_resp_callback, DISP_HOW_CC, &data,
		                  NULL, NULL),
		                  return S6A_FD_ERROR);
