...
mme_msg_rx_s6a_insert_subscriber_data_request(mme_msg_rx_family.Add({{"interface","s6a"},{"msg_type","insert_subscriber_data_request"}})),
...
mme_msg_tx_s6a_insert_subscriber_data_answer(mme_msg_tx_family.Add({{"interface","s6a"},{"msg_type","insert_subscriber_data_answer"}})),
...
void mmeStats::increment(mmeStatsCounter name,std::map<std::string,std::string> labels)
{
	...
	case mmeStatsCounter::MME_MSG_RX_S6A_INSERT_SUBSCRIBER_DATA_REQUEST:
	{
		mme_msg_rx_m->mme_msg_rx_s6a_insert_subscriber_data_request.Increment();
		if(labels.size() == 0) {
		break;
		}
		if(labels.size() == 1) {
		...
		if(it1 != metrics_map.end()) {
		    ...
		    mme_msg_rx_DynamicMetricObject1 *obj = mme_msg_rx_m->add_dynamic1("interface","s6a","msg_type","insert_subscriber_data_request",it->first, it->second);
		    auto p1 = std::make_pair(s1, obj);
		    metrics_map.insert(p1);
		    obj->counter.Increment();
		}
		...
		if(itf != metrics_map.end()) {
		    ...
		    mme_msg_rx_DynamicMetricObject2 *obj = mme_msg_rx_m->add_dynamic2("interface","s6a","msg_type","insert_subscriber_data_request",it1->first, it1->second, it2->first, it2->second);
		    auto p1 = std::make_pair(s1, obj);
		    metrics_map.insert(p1);
		    obj->counter.Increment();
		} 
		...
		if(itf != metrics_map.end()) {
		    ...
		    mme_msg_rx_DynamicMetricObject3 *obj = mme_msg_rx_m->add_dynamic3("interface","s6a","msg_type","insert_subscriber_data_request",it1->first, it1->second, it2->first, it2->second, it3->first, it3->second);
		    auto p1 = std::make_pair(s1, obj);
		    metrics_map.insert(p1);
		    obj->counter.Increment();
		}
	...
	case mmeStatsCounter::MME_MSG_TX_S6A_INSERT_SUBSCRIBER_DATA_ANSWER:
	{
		mme_msg_tx_m->mme_msg_tx_s6a_insert_subscriber_data_answer.Increment();
		if(labels.size() == 0) {
		break;
		}
		if(labels.size() == 1) {
		...
		if(it1 != metrics_map.end()) {
		    ...
		    mme_msg_tx_DynamicMetricObject1 *obj = mme_msg_tx_m->add_dynamic1("interface","s6a","msg_type","insert_subscriber_data_answer",it->first, it->second);
		    auto p1 = std::make_pair(s1, obj);
		    metrics_map.insert(p1);
		    obj->counter.Increment();
		}
		...
		if(itf != metrics_map.end()) {
		    ...
		    mme_msg_tx_DynamicMetricObject2 *obj = mme_msg_tx_m->add_dynamic2("interface","s6a","msg_type","insert_subscriber_data_answer",it1->first, it1->second, it2->first, it2->second);
		    auto p1 = std::make_pair(s1, obj);
		    metrics_map.insert(p1);
		    obj->counter.Increment();
		} 
		...
		if(itf != metrics_map.end()) {
		    ...
		    mme_msg_tx_DynamicMetricObject3 *obj = mme_msg_tx_m->add_dynamic3("interface","s6a","msg_type","insert_subscriber_data_answer",it1->first, it1->second, it2->first, it2->second, it3->first, it3->second);
		    auto p1 = std::make_pair(s1, obj);
		    metrics_map.insert(p1);
		    obj->counter.Increment();
		}
