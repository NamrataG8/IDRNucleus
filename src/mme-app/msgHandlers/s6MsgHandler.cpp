...
void S6MsgHandler::handleS6Message_v(IpcEMsgUnqPtr eMsg)
{
	...
	switch (msgData_p->msg_type)
	{
		...
		case msg_type_t::insert_subsdata_request:
			mmeStats::Instance()->increment(mmeStatsCounter::MME_MSG_RX_S6A_INSERT_SUBSCRIBER_DATA_REQUEST);
			handleInsertSubscriberDataRequest_v(std::move(eMsg),  msgData_p->ue_idx);
			break;
...
void S6MsgHandler::handleInsertSubscriberDataRequest_v(cmn::IpcEMsgUnqPtr eMsg, uint32_t ueIdx)
{
	log_msg(LOG_INFO, "Inside handleInsertSubscriberDataRequest_v ");

	utils::MsgBuffer* msgData_p = eMsg->getMsgBuffer();
	void* buf = msgData_p->getDataPointer();
	const idr_Q_msg_t* msgInfo_p = (idr_Q_msg_t *)(buf);
	...
	if(controlBlk_p == NULL)
	{
		log_msg(LOG_ERROR, "handleInsertSubscriberDataRequest_v: "
							"Failed to find UE context using idx %d",
							ueIdx);
		return;
	}
	// Fire Update Loc Answer event, insert cb to procedure queue
	SM::Event evt(IDR_FROM_HSS, cmn::IpcEMsgShPtr(std::move(eMsg)));
	controlBlk_p->addEventToProcQ(evt);
}
