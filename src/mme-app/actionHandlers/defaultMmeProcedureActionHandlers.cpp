...
/***************************************
* Action handler : default_insert_subscriber_data_req_handler
***************************************/
ActStatus ActionHandlers::default_insert_subscriber_data_req_handler(ControlBlock& cb)
{
	...
	const idr_Q_msg_t *idr_msg = static_cast<const idr_Q_msg_t*>(msgBuf->getDataPointer());

	sessionCtxt->setApnConfigProfileCtxId(idr_msg->apn_config_profile_ctx_id);
	DigitRegister15 ueMSISDN;
	ueMSISDN.convertFromBcdArray( reinterpret_cast<const uint8_t*>( idr_msg->MSISDN ));
	ue_ctxt->setMsisdn(ueMSISDN);
	ue_ctxt->setRauTauTimer(idr_msg->RAU_TAU_timer);
	ue_ctxt->setSubscriptionStatus(idr_msg->subscription_status);
	ue_ctxt->setNetAccessMode(idr_msg->net_access_mode);
	ue_ctxt->setAccessRestrictionData(idr_msg->access_restriction_data);
	ue_ctxt->setSubscribedApn(Apn_name(idr_msg->selected_apn));

	for (int i = 0; i < idr_msg->supp_features_list.count; i++)
	{
		if (idr_msg->supp_features_list.supp_features[i].feature_list_id == 2)
		{
			log_msg(LOG_DEBUG,"received feature_list_id2 from hss %u %u ",idr_msg->supp_features_list.supp_features[i].feature_list_id, idr_msg->supp_features_list.supp_features[i].feature_list);
			ue_ctxt->setHssFeatList2(
					idr_msg->supp_features_list.supp_features[i]);
			break;
		}
	}
	...
	struct AMBR ambr;
	ambr.max_requested_bw_dl = idr_msg->max_requested_bw_dl;
	ambr.max_requested_bw_ul = idr_msg->max_requested_bw_ul;
	if(idr_msg->extended_max_requested_bw_dl > 0 || idr_msg->extended_max_requested_bw_ul > 0)
	{
		ambr.ext_max_requested_bw_dl = idr_msg->extended_max_requested_bw_dl;
		ambr.ext_max_requested_bw_ul = idr_msg->extended_max_requested_bw_ul;
	}
	...
	struct PAA pdn_addr;
	pdn_addr.pdn_type = 1;
	pdn_addr.ip_type.ipv4.s_addr = ntohl(idr_msg->static_addr); // network byte order
